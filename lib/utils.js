...
module.exports = {        
    ...
    insertNewCardsJSON: async (cards) => (await (() => (
        new Promise((resolve, reject) => (connect().then(client => {
            //add new collection
            client.db(dbName).collection("latestCards").insertMany(
                cards
            )
            .then(function (doc) {
                close(client);
                resolve("Successfully Saved New Cards ");
            })
            .catch(function (error) {
                console.log(error);
                close(client);
                resolve("COULD NOT INSERT NEW CARDS IN DB");
            });
        }
        )))))()),
};